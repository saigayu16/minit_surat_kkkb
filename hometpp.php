...
// 1. SEMAK SESI & ROLE TPP
// Pastikan hanya user dengan role 'tpp' sahaja boleh akses halaman ini
if (!isset($_SESSION['user_role']) || $_SESSION['user_role'] !== 'tpp') {
    header("Location: login.php");
    exit;
}

$user_name = $_SESSION['user_name'] ?? 'Timbalan Pengarah';
$user_role = "Timbalan Pengarah Pengurusan (TPP)";

// 2. KIRA STATISTIK KHAS TPP (Menggunakan PDO)
$total_perlu_sahkan = 0;
$total_selesai = 0;
$total_semua = 0;

// Kira Dokumen Menunggu Pengesahan TPP 
$count_wait = $pdo->query("SELECT COUNT(*) as total FROM minit_surat WHERE target_role = 'tpp' AND status NOT LIKE '%Sudah Disahkan%'");
if($count_wait) {
    $total_perlu_sahkan = $count_wait->fetch(PDO::FETCH_ASSOC)['total'];
}

// Kira Dokumen Selesai Disahkan TPP
$count_done = $pdo->query("SELECT COUNT(*) as total FROM minit_surat WHERE target_role = 'tpp' AND status LIKE '%Sudah Disahkan%'");
if($count_done) {
    $total_selesai = $count_done->fetch(PDO::FETCH_ASSOC)['total'];
}

// Kira Jumlah Keseluruhan Dokumen TPP
$count_all = $pdo->query("SELECT COUNT(*) as total FROM minit_surat WHERE target_role = 'tpp'");
if($count_all) {
...
<head>
    ...
    <title>Dashboard TPP - Sistem Minit Digital</title>
...
<nav class="navbar">
    <h2><i class="fa-solid fa-briefcase"></i> Sistem Minit Digital</h2>
    <div class="user-info">
...
<div class="container">
    ...
    <div class="table-title">
        ...
        Senarai Dokumen Semakan & Pengesahan TPP
    </div>
    ...
    <div class="table-container">
        <table>
            ...
            <tbody>
                ...
                $res = $pdo->query("SELECT id, no_rujukan, tarikh_terima, daripada, kepada, perkara, perkara_surat, kolej, didaftarkan_oleh, status, fail_surat, tempoh_tindakan, tandatangan_fail, tarikh_disahkan, target_role, catatan, tandatangan, arahan_pilihan, maklum_kepada, tandatangan_data, drive_file_id, arahan, created_at, staf_dimaklumkan FROM minit_surat WHERE target_role = 'tpp' ORDER BY id DESC");
                ...
                if ($res) {
                    ...
                    if (count($rows) > 0) {
                        foreach($rows as $row) {
                            ...
                            // Memastikan ia menyokong lajur 'perkara' atau 'perkara_surat'
                        ...
                        echo "<tr><td colspan='6' style='text-align:center; padding: 30px; color: var(--text-muted);'>📂 Tiada dokumen untuk disemak oleh TPP buat masa ini.</td></tr>";
                    }
                } else {
                    echo "<tr><td colspan='6' style='text-align:center; padding: 30px; color: var(--text-muted);'>📂 Tiada dokumen untuk disemak oleh TPP buat masa ini.</td></tr>";
                }
